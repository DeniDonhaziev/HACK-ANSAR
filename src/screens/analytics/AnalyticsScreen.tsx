...
export function AnalyticsScreen() {
  const { clients, visits, tasks, expenses, salesPlans, addSalesPlan } = useDataStore();
    ...
    <ScreenContainer title={COPY.analytics.title} subtitle={COPY.analytics.subtitle}>
      {isEmpty ? (
        ...
        <>
          ...
          <SectionHeader title="План продаж" action={{ label: plan ? 'Изменить' : 'Создать', onPress: () => setModalVisible(true) }} />
          {plan ? (
            <Card>
      ...
      <FormModal visible={modalVisible} title="План продаж" onClose={() => setModalVisible(false)}>
