...
export function MapScreen() {
  ...
  return (
    <ScreenContainer title="Карта и маршрут" scrollable={false}>
      {Platform.OS !== 'web' && MapView && clientsWithCoords.length > 0 ? (
        ...
        <Card variant="outlined">
          ...
          {clients.map((c) => (
            <Text key={c.id} style={styles.clientItem}>{c.name} — {c.address}</Text>
          ))}
        </Card>
      )}
...
const styles = StyleSheet.create({
  mapContainer: { height: 280, borderRadius: 16, overflow: 'hidden', marginBottom: spacing.md },
  map: { flex: 1 },
  list: { maxHeight: 280 },
