...
const variantColors: Record<BadgeVariant, { bg: string; text: string }> = {
  ...
  primary: { bg: colors.accentLight, text: colors.accentDark },
...
export function StatCard({ title, value, subtitle, color = colors.accentDark, icon, onPress }: StatCardProps) {
  return (
...
export function Fab({ onPress, icon = 'add' }: FabProps) {
  ...
  const bottom = spacing.lg + (Platform.OS !== 'web' ? insets.bottom : 0);
...
const styles = StyleSheet.create({
  ...
  sectionAction: { ...typography.label, color: colors.accentDark },
  fab: {
